...
class ForegroundReader:
    ...
    def show(self, frame: np.array, winname: str='show'):
        if self.frame_cnt == 1:
    ...
    @staticmethod
    def get_writer(frame: np.array, output_path: str, fps: Optional[int]=20):
        ext = "mp4v" if output_path.endswith('mp4') else "XVID"
    ...
    def __repr__(self):
        text = f"{'-'*12} {os.path.basename(self.source)} {'-'*12}"
        new_line = '\n'
        total_frame = self.total_frames
        img = self.video_capture.read()[1]
        fps = self.video_capture.get(cv2.CAP_PROP_FPS)
        text += f'{new_line}Total_Frames: {total_frame}'
        text += f'{new_line}H, W, C: {img.shape}'
        text += f'{new_line}FPS: {fps}'
        return text
